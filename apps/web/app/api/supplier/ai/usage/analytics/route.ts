...
const ALL_FEATURES = ["SUPPLIER_CHAT", "SUPPLIER_DIGEST"] as const;
...
export async function GET(req: Request) {
  try {
    ...
    if (!hasTier(user.supplier.planTier, ROUTE_TIER.AI_USAGE_ANALYTICS)) {
      return NextResponse.json(
        {
          error: "Professional plan required",
          message:
            "AI usage analytics require a Professional plan. Upgrade in Settings to access this feature.",
          upgradeUrl: "/supplier/settings",
          currentTier: user.supplier.planTier,
    ...
    const { id: supplierId } = user.supplier;
    ...
    const timeSeries = Array.from(dateMap.entries()).map(
      ([date, features]) => {
        ...
        return {
          date,
          SUPPLIER_CHAT: features.SUPPLIER_CHAT.count,
          SUPPLIER_DIGEST: features.SUPPLIER_DIGEST.count,
    ...
    const perUser = Array.from(userMap.entries())
      .map(([userId, data]) => ({
        ...
        name:
          userId === "__system__"
            ? "System Jobs"
            : (userNameMap.get(userId) ?? "Unknown"),
    ...
    console.error("Supplier AI usage analytics error:", error);
    return NextResponse.json(
